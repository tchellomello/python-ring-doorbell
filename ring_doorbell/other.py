...
class RingOther(RingGeneric):
    ...
    def update_health_data(self) -> None:
        """Update health attrs."""
        self._ring.auth._run_async_on_event_loop(self.async_update_health_data())
    ...
    def doorbell_volume(self, value: int) -> None:
        self._ring.auth._run_async_on_event_loop(self.async_set_doorbell_volume(value))
    ...
    def keep_alive_auto(self, value: float) -> None:
        self._ring.auth._run_async_on_event_loop(self.async_set_keep_alive_auto(value))
    ...
    def mic_volume(self, value: int) -> None:
        self._ring.auth._run_async_on_event_loop(self.async_set_mic_volume(value))
    ...
    def voice_volume(self, value: int) -> None:
        self._ring.auth._run_async_on_event_loop(self.async_set_voice_volume(value))
    ...
    def clip_length_max(self) -> Optional[int]:
        return self._ring.auth._run_async_on_event_loop(self.async_clip_length_max())

    @clip_length_max.setter
    def clip_length_max(self, value: int) -> None:
        self._ring.auth._run_async_on_event_loop(self.async_set_clip_length_max(value))

    async def async_clip_length_max(self) -> Optional[int]:
    ...
    def allowed_users(self) -> Optional[List[Dict[str, Any]]]:
        ...
        return self._ring.auth._run_async_on_event_loop(self.async_allowed_users())

    async def async_allowed_users(self) -> Optional[List[Dict[str, Any]]]:
        """Return list of users allowed or invited to access"""
        if self.kind in INTERCOM_KINDS:
    ...
    def open_door(self, user_id: int = -1) -> bool:
        """Open the door"""
        return self._ring.auth._run_async_on_event_loop(self.async_open_door(user_id))
    ...
    def invite_access(self, email: str) -> bool:
        """Invite user"""
        return self._ring.auth._run_async_on_event_loop(self.async_invite_access(email))
